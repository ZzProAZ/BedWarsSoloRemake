...
use dctxdev\tw\libs\muqsit\invmenu\inventory\InvMenuInventory;
...
use pocketmine\event\entity\{EntityMotionEvent, EntityDamageEvent, ProjectileHitEntityEvent};
use pocketmine\event\player\PlayerCommandPreprocessEvent;
use pocketmine\event\entity\EntityRegainHealthEvent;
use pocketmine\event\entity\EntityDamageByEntityEvent;
use pocketmine\event\block\BlockBreakEvent;
use pocketmine\event\block\{BlockPlaceEvent, BlockUpdateEvent, LeavesDecayEvent};
use pocketmine\event\inventory\{InventoryPickupItemEvent, InventoryTransactionEvent, InventoryOpenEvent, InventoryCloseEvent};
...
use dctxdev\tw\math\{Vector3, Generator, Dinamite as TNT, Bedbug, Golem, Fireball};
use dctxdev\tw\SkyWars;
use fw\Fireworks;
use dctxdev\tw\ServerManager;
use dctxdev\tw\libs\muqsit\invmenu\{
    InvMenu
};
use pocketmine\nbt\tag\CompoundTag;
use pocketmine\nbt\tag\StringTag;
use pocketmine\nbt\tag\ByteArrayTag; 
use dctxdev\tw\libs\muqsit\invmenu\transaction\{DeterministicInvMenuTransaction, InvMenuTransaction, InvMenuTransactionResult};
...
class Arena implements Listener {
    ...
    public function setColorTag($player){
        $color = ["red" => "§c", "blue" => "§1", "green" => "§a", "yellow" => "§e", "aqua" => "§b", "white" => "§f", "pink" => "§d", "gray" => "§7"];
        $player->setNametag($color[$this->getTeam($player)]." ".  $player->getName());
    }
    ...
    public function setTeam($player, $index){
        ...
        if(in_array($index, [3])){
            $this->yellowteam[$player->getName()] = $player; 
        } 
        if(in_array($index, [4])){
            $this->greenteam[$player->getName()] = $player; 
        }
        if(in_array($index, [5])){
            $this->aquateam[$player->getName()] = $player;
        }
        if(in_array($index, [6])){
            $this->whiteteam[$player->getName()] = $player;
        }
        if(in_array($index, [7])){
            $this->pinkteam[$player->getName()] = $player;
        }
        if(in_array($index, [8])){
            $this->grayteam[$player->getName()] = $player;
        }
    }
    ...
    public function treasureStatus($team){
        ...
        if(($tr = $this->level->getBlockAt($vc->x, $vc->y, $vc->z))->getId() !== 0){
            $status = "§aBed Alive";
        } else {
            $status = "§cBed Broken";
        }
    ...
    public function teamStatus($team){
        ...
        if(($tr = $this->level->getBlockAt($vc->x, $vc->y, $vc->z)) instanceof Bed){
                $status = $t == $this->getTeam($mate) ? "§b✔ " : "§b✔";
        } else {
       $count = 0;
        foreach($this->players as $mate){
     ...
     if($count <= 0){
          $status = "§c✘";
             ...
             $status = $t == $this->getTeam($mate) ? "§a" . $count . " " : "§a" . $count . "";
        }
     }
    }
        return $status;
    ...
    public function getTeam(Player $player){
        ...
        if(isset($this->yellowteam[$player->getName()])){
            return "yellow";
        }
        if(isset($this->greenteam[$player->getName()])){
            return "green";
        }
        if(isset($this->aquateam[$player->getName()])){
            return "aqua";
        }
        if(isset($this->whiteteam[$player->getName()])){
            return "white";
        }
        if(isset($this->pinkteam[$player->getName()])){
            return "pink";
        }
        if(isset($this->grayteam[$player->getName()])){
            return "gray";
        }
        return "";
    }
    
    public function hasTreasure(Player $player){
    ...
    public function joinToArena(Player $player) {
        ...
        ");
        //$player->getInventory()->setItem(4, Item::get(399, 0, 1)->setCustomName(TextFormat::GREEN . "Locker"));

        self::$bossbar = (new BossBar())->setPercentage(1);
        self::$bossbar->setTitle(TextFormat::YELLOW . TextFormat::BOLD . "Playing" . TextFormat::RED . "Bed" . TextFormat::WHITE . "Wars" . TextFormat::GOLD . "Dctx" . TextFormat::YELLOW . "In " . TextFormat::GREEN . "servername" . TextFormat::RESET . TextFormat::WHITE);
        ...
        $this->broadcastMessage("§b{$player->getNametag()} §7joined. ".count($this->players)."/{$this->data["slots"]}!");
    }
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = false) {
        ...
        if($this->plugin->config["waterdog"]["enabled"]) {
            ServerManager::transferPlayer($player, $this->plugin->config["waterdog"]["lobbyServer"]);
        }
        ...
        if($this->phase == self::PHASE_GAME){
            ...
            if($count <= 0){
                ...
                $this->broadcastMessage("$color[$team] §fwas §cELIMINATED!");
            }
            if($team == "red"){
            ...
            if($team == "yellow"){
                unset($this->yellowteam[$player->getName()]);
                $player->setNameTag($player->getDisplayName());
                $player->setDisplayName($player->getDisplayName());
            }
            if($team == "blue"){
                unset($this->greenteam[$player->getName()]);
                $player->setNameTag($player->getDisplayName());
                $player->setDisplayName($player->getDisplayName());
            }
            if($team == "aqua"){
                unset($this->aquateam[$player->getName()]);
                $player->setNameTag($player->getDisplayName());
                $player->setDisplayName($player->getDisplayName());
            }
            if($team == "white"){
                unset($this->whiteteam[$player->getName()]);
                $player->setNameTag($player->getDisplayName());
                $player->setDisplayName($player->getDisplayName());
            }
            if($team == "pink"){
                unset($this->pinkteam[$player->getName()]);
                $player->setNameTag($player->getDisplayName());
                $player->setDisplayName($player->getDisplayName());
            }
            if($team == "gray"){
                unset($this->grayteam[$player->getName()]);
                $player->setNameTag($player->getDisplayName());
                $player->setDisplayName($player->getDisplayName());
            }
        }
        ...
        if(!$death) {
            $this->broadcastMessage("§b{$player->getDisplayName()} left");
        }
    ...
    public function spectator(Player $player) {
        ...
        $player->getInventory()->setItem(4, Item::get(Item::COMPASS, 0, 1)->setCustomName("§bTeleporter"));
        $team = $this->getTeam($player);
        if($this->phase == self::PHASE_GAME){
            $count = 0;
            foreach($this->players as $mate){
            ...
            if($count <= 0){
                ...
                $this->broadcastMessage("$color[$team] §r§cwas §l§cELIMINATED!");
            }
            if($team == "red"){
                unset($this->redteam[$player->getName()]);
                $player->setNameTag($player->getDisplayName());
    ...
    public function setSword($player, $sword){
        if(!($player instanceof Player)) return;
        if(!$sword instanceof Sword) return;
        $team = $this->getTeam($player);
        $enchant = null;
        if(isset($this->teamsharpness[$team])){
            if($this->teamsharpness[$team] > 1){
            $enchant = new EnchantmentInstance(Enchantment::getEnchantment(Enchantment::SHARPNESS), ($this->teamsharpness[$team] - 1));
            }
        }
        if($enchant !== null){
    ...
    public function setArmor($player){
        ...
        if(isset($this->teamprotection[$team])){
            ...
            $enchant = new EnchantmentInstance(Enchantment::getEnchantment(Enchantment::PROTECTION), ($this->teamprotection[$team] - 1));
            }
        }
        $color = null;
        if($team == "red"){
    ...
    public function prepareWorld(){
        ...
        foreach($this->level->getTiles() as $tile){
            if($tile instanceof Furnace){
                $nbt = Entity::createBaseNBT(new Vector3($tile->x + 0.5, $tile->y + 1, $tile->z + 0.5), null);
            ...
            if($tile instanceof EnchantTable){
                $nbt = Entity::createBaseNBT(new Vector3($tile->x + 0.5, $tile->y + 5, $tile->z + 0.5), null);
                $path = $this->plugin->getDataFolder() . "diamond.png";
                $skin = $this->plugin->getSkinFromFile($path);
                $nbt->setTag(new CompoundTag('Skin', [
                new StringTag('Data', $skin->getSkinData()),
                ...
                if($t == "red"){
                    foreach($this->redteam as $red){
                        if($red instanceof Player){
                            $red->addTitle("§l§cBED DESTORYED", "§r§fyou will no longer respawn");
                ...
                if($t == "blue"){
                    foreach($this->blueteam as $blue){
                        if($blue instanceof Player){
                            $blue->addTitle("§l§cBED DESTORYED", "§r§fyou will no longer respawn");;
                ...
                if($t == "yellow"){
                    foreach($this->yellowteam as $yellow){
                        if($yellow instanceof Player){
                            $yellow->addTitle("§l§cBED DESTORYED", "§r§fyou will no longer respawn");
                ...
                if($t == "green"){
                    foreach($this->greenteam as $green){
                        if($green instanceof Player){
                            $green->addTitle("§l§cBED DESTORYED", "§r§fyou will no longer respawn");
                            $this->addSound($green, 'mob.wither.death', 1); 
                        }
                    }
                }
                if($t == "aqua"){
                    foreach($this->aquateam as $aqua){
                        if($aqua instanceof Player){
                            $aqua->addTitle("§l§cBED DESTORYED", "§r§fyou will no longer respawn");
                            $this->addSound($aqua, 'mob.wither.death', 1);
                        }
                    }
                }
                if($t == "white"){
                    foreach($this->whiteteam as $white){
                        if($white instanceof Player){
                            $white->addTitle("§l§cBED DESTORYED", "§r§fyou will no longer respawn");
                            $this->addSound($white, 'mob.wither.death', 1);
                        }
                    }
                }
                if($t == "pink"){
                    foreach($this->pinkteam as $pink){
                        if($pink instanceof Player){
                            $pink->addTitle("§l§cBED DESTORYED", "§r§fyou will no longer respawn");
                            $this->addSound($pink, 'mob.wither.death', 1);
                        }
                    }
                }
                if($t == "gray"){
                    foreach($this->grayteam as $gray){
                        if($gray instanceof Player){
                            $gray->addTitle("§l§cBED DESTORYED", "§r§fyou will no longer respawn");
                            $this->addSound($gray, 'mob.wither.death', 1);
                        }
                    }
                }
            }
    ...
    public function checkTeam(){
        ...
        if(count($this->aquateam) <= 0){
            $pos = Vector3::fromString($this->data["treasure"]["aqua"]);
            if(($bed = $this->level->getBlockAt($pos->x, $pos->y, $pos->z)) instanceof Bed){
                $next = $bed->getOtherHalf();
                $this->level->setBlock($bed, Block::get(0));
                $this->level->setBlock($next, Block::get(0));
        ...
        if(count($this->whiteteam) <= 0){
            $pos = Vector3::fromString($this->data["treasure"]["white"]);
            if(($bed = $this->level->getBlockAt($pos->x, $pos->y, $pos->z)) instanceof Bed){
                $next = $bed->getOtherHalf();
                $this->level->setBlock($bed, Block::get(0));
                $this->level->setBlock($next, Block::get(0));
        ...
        if(count($this->pinkteam) <= 0){
            $pos = Vector3::fromString($this->data["treasure"]["pink"]);
            if(($bed = $this->level->getBlockAt($pos->x, $pos->y, $pos->z)) instanceof Bed){
                $next = $bed->getOtherHalf();
                $this->level->setBlock($bed, Block::get(0));
                $this->level->setBlock($next, Block::get(0));
        ...
        if(count($this->grayteam) <= 0){
            $pos = Vector3::fromString($this->data["treasure"]["gray"]);
            if(($bed = $this->level->getBlockAt($pos->x, $pos->y, $pos->z)) instanceof Bed){
                $next = $bed->getOtherHalf();
                $this->level->setBlock($bed, Block::get(0));
                $this->level->setBlock($next, Block::get(0));
    ...
    public function destroyTreasure($team, $player = null){
        if(!isset($this->data["treasure"][$team])) return;
    ...
    if($player instanceof Player){
        $this->level->broadcastLevelSoundEvent(new Vector3($player->getX(), $player->getY(), $player->getZ()), LevelSoundEventPacket::SOUND_NOTE);
    }
            $this->level->setBlock($next, Block::get(0));
        }
        $c = null;
        ...
        $tn = ucwords($team);
        if($player instanceof Player){
            $this->broadcastMessage("§l§fBED DECONTRUCTION > §r§b{$c}{$tn} bed was §cDestroyed §bby {$player->getNametag()}");
            if(isset($this->broken[$player->getId()])){
        ...
        foreach($this->players as $p){
            if($p instanceof Player && $this->getTeam($p) == $team){
                $p->sendTitle("§l§eBED DESTORYED", "§r§ayou will no longer respawn");
    ...
    public function Wins(string $team) {
        ...
        $this->broadcastMessage("§l§b$teamName[$team]§l§b WINNERS!");  
    ...
    public function TopKills(Player $player) : void{
     if($player instanceof Player) {
        $msg = "§l§bTOP FINNAL KILLS
        \n \n \n";
        $player->sendMessage($msg);
        $kconfig = new Config($this->plugin->getDataFolder()."pdata/finalkills.yml", Config::YAML, [$player->getName() => 0]);
        ...
        foreach($kills as $playerName => $killCount){
            ...
            if($i < 4 && $killCount){
                switch($i){
                    case 1:
                        $satu = "§b#1 §2".$playerName." - §f".$killCount."\n \n \n";
                        $player->sendMessage($satu);
                        break;
                    case 2:
                        $dua = "§b#2 §2".$playerName." - §f".$killCount."\n \n \n";
                        $player->sendMessage($dua);
                        break;
                    case 3:
                        $tiga = "§b#3 §2".$playerName." - §f".$killCount."\n \n \n";
    ...
    public function draw() {
        ...
        $this->milk = [];
        $this->inChest = [];
        $this->broadcastMessage("§l§bGAME OVER", self::MSG_TITLE); 
        $this->phase = self::PHASE_RESTART;
    }
    /**
     * @param Player $player
        ...
        if($pro instanceof \pocketmine\entity\projectile\Arrow){
            if($owner instanceof Player && $hitEntity instanceof Player){
                if($this->inGame($owner)){
                    $owner->sendMessage("§f{$hitEntity->getDisplayName()} is now {$hitEntity->getHealth()} hearts");
    ...
    public function onConsume(\pocketmine\event\player\PlayerItemConsumeEvent $event){
        $player = $event->getPlayer();
        $item = $event->getItem();
        ...
        if($this->inGame($player)){
            ...
            if($item->getId() == Item::BUCKET && $item->getDamage() == 1){
                ...
                $player->sendMessage("§aTrap wont be affected on §c30 §bseconds");
            }
        }
    }
    ...
    public function blockUpdateEvent(BlockUpdateEvent $event){
        ...
        if($block->getLevel()->getFolderName() == $this->level->getFolderName()){
            $event->setCancelled();
        }
    }
    ...
    public function onMove(PlayerMoveEvent $event) {
        ...
        if($this->inGame($player)){
            ...
            if($this->phase == self::PHASE_GAME){
                ...
                foreach(["red", "blue", "yellow", "green", "aqua", "white", "pink", "gray"] as $teams){
    ...
    public function projectileOnHit(\pocketmine\event\entity\ProjectileHitEvent $event){
        $pro = $event->getEntity();
    ...
    public function onChat(PlayerChatEvent $event){
        ...
        if($f == "?"){
            $msg = str_replace("?", "", $msg);
            if(trim($msg) !== ""){
                $this->broadcastMessage("§6SHOUT §r§f> §r{$player->getDisplayName()}: §7{$msg}");
            }
        } else {
            $team = $this->getTeam($player);
            foreach($this->players as $pt){
                if($this->getTeam($pt) == $team){
                    $pt->sendMessage("§eTEAM §r§f> §r{$player->getDisplayName()}: §7{$msg}");
                }
            }
        }
        $event->setCancelled();
			...
			if($source instanceof Player){
			    if($this->inGame($source)){
			        if($action instanceof SlotChangeAction){
			            ...
			            if($action->getInventory() instanceof \pocketmine\inventory\ArmorInventory){
    ...
    public function onBreak(BlockBreakEvent $event){
        ...
        if($this->inGame($player)){
            ...
            if($block instanceof Bed){
                ...
                $aqua = $this->data["treasure"]["aqua"];
                $white = $this->data["treasure"]["white"];
                $pink = $this->data["treasure"]["pink"];
                $gray = $this->data["treasure"]["gray"];
                if(in_array(($pos = (new Vector3($next->x, $next->y, $next->z))->__toString()), [$red, $blue, $yellow, $green, $aqua, $white, $pink, $gray])){
                    $team = null;
                    if($pos == $red){
                        $team = "red";
                    }
                   ...
                   $player->sendMessage("§cWarning: you can't break your team");
                    }
                    return; 
    ...
    public function onPlace(BlockPlaceEvent $event){
        ...
        if($this->inGame($player)){
            ...
            if($block->asVector3()->distance($lv) < 6){
                $event->setCancelled();
                $player->sendMessage("§cWarning: you can't place block in here");
            } else {
                $pos = new Vector3(floor($block->x) + 0.5, $block->y, floor($block->z) + 0.5); 
            ...
            if($block->getId() == Block::TNT){
                $event->setCancelled();
                $pos = new Vector3(floor($block->getX()), floor($block->getY()), floor($block->getZ()));
    ...
    public function spawnFireball($pos, $level, $player){
		...
		$entity->setMotion($player->getDirectionVector()->normalize()->multiply(0.5));
		$entity->spawnToAll(); 
    }
    ...
    public function onInteract(PlayerInteractEvent $event) {
        ...
        if($player->getLevel()->getFolderName() == $this->level->getFolderName()){
            ...
            if($this->phase == self::PHASE_RESTART){
                if($action == $event::RIGHT_CLICK_BLOCK || $action == $event::RIGHT_CLICK_AIR){
                if($item->getId() == Item::BED && $item->getDamage() == 14){
                    $player->getInventory()->setHeldItemIndex(1);
                    SkyWars::instance()->joinToRandomArena($player);
                } 
                  if($item->getId() == Item::BED && $item->getDamage() == 14){
                    if($this->plugin->config["waterdog"]["enabled"]) {
                        ServerManager::transferPlayer($player, $this->plugin->config["waterdog"]["lobbyServer"]);
                ...
                if ($action == $event::RIGHT_CLICK_BLOCK || $action == $event::RIGHT_CLICK_AIR) {
                    ...
                    if ($this->inGame($player)) {
                        ...
                        if ($item->getId() == Item::FIRE_CHARGE) {
                            $this->spawnFireball($player->add(0, $player->getEyeHeight(), 0), $player->level, $player);
                        ...
                        if ($action == $event::RIGHT_CLICK_BLOCK) {
                            ...
                            if ($item->getId() == Item::SPAWN_EGG && $item->getDamage() == 14) {
                                $this->spawnGolem($block->add(0, 1, 0), $player->level, $player);
                                $ih->setCount($ih->getCount() - 1);
                                $player->getInventory()->setItemInHand($ih);
                                $event->setCancelled();
                            }
                        ...
                        if ($item->getId() == Item::COMPASS) {
                            ...
                            if ($item->equalsExact($player->getInventory()->getItemInHand())) {
                                if (is_null($detected)) {
                                    $player->sendMessage("Can't find nearest player");
                                } else {
                                    $player->sendMessage("Set compass to nearest player");
                                }
                            }
                        }
                    }
    ...
    public function onDamage(EntityDamageEvent $event) {
        ...
        if($this->phase == self::PHASE_GAME){
            if($event instanceof EntityDamageByEntityEvent){
                ...
                if($dmg instanceof Player && $player->getNameTag() == "§l§eITEM SHOP\n§r§bLEFT CLICK"){
                    if($this->inGame($dmg)){
                ...
                if($dmg instanceof Player && $player->getNameTag() == "§l§eTEAM UPGRADE\n§r§bLEFT CLICK"){
                    if($this->inGame($dmg)){
                        if($dmg->distance($player) < 5){ 
        ...
        if($player instanceof Player){
            if($this->inGame($player)){
            if($this->phase == self::PHASE_GAME){
                if($cause == $event::CAUSE_ENTITY_EXPLOSION || $cause == $event::CAUSE_BLOCK_EXPLOSION){
                ...
                if($event instanceof EntityDamageByEntityEvent){
                    if($this->inGame($event->getDamager())){
                        if($this->getTeam($event->getDamager()) !== $this->getTeam($player)){
                            ...
                            $this->plugin->damaged[$player->getName()] = $player;
                            $this->plugin->lastDamager[$player->getName()] = $event->getDamager();
                            $this->plugin->lastTime[$player->getName()] = 10;
                        } else {
                            $event->setCancelled();
                        }
                    }
                ...
                if(isset($this->plugin->lastDamager[$player->getName()])){
                    $damager = $this->plugin->lastDamager[$player->getName()];
                    if($damager instanceof Player && $player instanceof Player){
                        if($player->getHealth() - $event->getFinalDamage() <= 0){
                            $event->setCancelled();
                            if(!$this->hasTreasure($player)){
                            $this->plugin->addFinalKill($damager);
                            //FW::getInstance()->spawnRocket($damager, Fireworks::TYPE_SMALL_SPHERE, $rcolor);
                            }
                            $this->plugin->addExp($player, rand(5,10));
                            $this->plugin->addKill($damager);
                            $this->plugin->death($player);
                            $player->getArmorInventory()->clearAll();
                            //$player->getInventory()->dropContents($player->getLevel(), $player->asVector3());
                            foreach($this->level->getPlayers() as $pl){
                                $msg = $this->plugin->getKillMsg($damager, $player);
                                if(!$this->hasTreasure($player)){
                                    $txt = "§l§eFINAL KILL! ";
                                } else {
                                    $txt = "";
                                }
                                $pl->sendMessage("§b {$txt}{$msg}");
                               $this->addSound($player, 'random.levelup', 1);
                            }
                            if(!$this->hasTreasure($player)){
                       ...
                       switch ($event->getCause()) {
                           case EntityDamageEvent::CAUSE_FALL:
                               ...
                               if(!$this->hasTreasure($player)){
                                   $txt = "§l§eFINAL KILL! ";
                                   $this->spectator($player);
                                   $this->broadcastMessage("§b{$txt}§r{$player->getDisplayName()} §bfell into void");
                               }else{
                                   $this->startRespawn($player);
                                   $this->broadcastMessage("§b{$txt}§r{$player->getDisplayName()} §bfell into void");
                                   $txt = "";
                   ...
                   case EntityDamageEvent::CAUSE_FIRE:
                        ...
                        if(!$this->hasTreasure($player)){
                            $txt = "§l§eFINAL KILL! ";
                            $this->spectator($player);
                            $this->broadcastMessage("§b{$txt}§r{$player->getDisplayName()} §bwent up in flame");
                        } else {
                            $this->startRespawn($player);
                            $this->broadcastMessage("§b{$txt}§r{$player->getDisplayName()} §bwent up in flame");
                            $txt = "";
                   ...
                   case EntityDamageEvent::CAUSE_ENTITY_EXPLOSION:
                        ...
                        if(!$this->hasTreasure($player)){
                            $txt = "§l§eFINAL KILL! ";
                            $this->spectator($player);
                            $this->broadcastMessage("§b{$txt}§r{$player->getDisplayName()} §Exploded");
                        } else {
                            $this->startRespawn($player);
                            $this->broadcastMessage("§b{$txt}§r{$player->getDisplayName()} §bExploded");
                            $txt = "";
                        }  
                        break;
                }
    ...
    public function upgradeGenerator($team, $player){
        ...
        foreach($this->players as $t){
            if($this->getTeam($t) == $team){
                ...
                $t->sendMessage("{$player->getDisplayName()} §ahas bought §bgenerator §alevel §c" . $lvl);
    ...
    public function upgradeArmor($team, $player){
        ...
        foreach($this->players as $pt){
            if($this->getTeam($pt) == $team){
                ...
                $this->setArmor($pt);
		        $pt->sendMessage("{$player->getDisplayName()} §ahas bought §bresistance §alevel §c" . $lvl);
            }
        }
    }
    ...
    public function upgradeHaste($team, $player){
		...
		foreach($this->players as $pt){
		    if($this->getTeam($pt) == $team){
		        ...
		        $this->addSound($pt, 'random.levelup', 1);
		        $pt->sendMessage("{$player->getDisplayName()} §ahas bought §bhaste §alevel §c" . $lvl);
		    }
		}
    }
    ...
    public function upgradeSword($team, $player){
		...
		foreach($this->players as $pt){
		    if($this->getTeam($pt) == $team){
		        ...
		        $pt->sendMessage("{$player->getDisplayName()} §ahas bought §bsharpness");
		    }
		}
    }
    ...
    public function upgradeHeal($team, $player){
		...
		foreach($this->players as $pt){
		    if($this->getTeam($pt) == $team){
		        $this->addSound($pt, 'random.levelup', 1);
		        $pt->sendMessage("{$player->getDisplayName()} §ahas bought §bheal pool");
		    }
		}
    } 
	...
	public function upgradeMenu($player){
     ...
     $level->broadcastLevelSoundEvent(new Vector3($player->getX(), $player->getY(), $player->getZ()), LevelSoundEventPacket::SOUND_NOTE);
		        ...
		        foreach($this->players as $pt){
		            if($this->getTeam($pt) == $team){
		                $pt->sendMessage("{$player->getDisplayName()} §ahas bought §bMiner Fatigue Trap");
		            }
		        } 
		        $this->traps[$team]++;
            } else {
	...
	public function shopMenu($player){
     ...
     $level->broadcastLevelSoundEvent(new Vector3($player->getX(), $player->getY(), $player->getZ()), LevelSoundEventPacket::SOUND_NOTE); 
        ...
        if($item instanceof Sword && $in == "§aDiamond Sword"){
            if($pinv->contains(Item::get(Item::EMERALD, 0, 4))){
                $pinv->removeItem(Item::get(Item::EMERALD, 0, 4));
                     $level = $player->getLevel();
     ...
     $level->broadcastLevelSoundEvent(new Vector3($player->getX(), $player->getY(), $player->getZ()), LevelSoundEventPacket::SOUND_NOTE);
	    ...
	    // Main Menu //
	    $inv->setItem(1, Item::get(Block::WOOL, $meta[$team], 1)->setCustomName("§fBlocks"));
	    $inv->setItem(2, Item::get(Item::GOLDEN_SWORD, 0, 1)->setCustomName("§fMelee"));
	    $inv->setItem(3, Item::get(Item::CHAINMAIL_BOOTS, 0, 1)->setCustomName("§fArmor"));
	    $inv->setItem(4, Item::get(Item::STONE_PICKAXE, 0, 1)->setCustomName("§fTools"));
	    $inv->setItem(5, Item::get(Item::BOW, 0, 1)->setCustomName("§fBow & Arrow"));
	    $inv->setItem(6, Item::get(Item::BREWING_STAND, 0, 1)->setCustomName("§fPotions"));
	    $inv->setItem(7, Item::get(Block::TNT, 0, 1)->setCustomName("§fUtility"));
	    // Block Menu //
    ...
    public function manageShop($player, $inv, $type){
        ...
        // SWORD //
        if($type == "§fMelee"){
        $inv->setItem(19, Item::get(Item::STONE_SWORD, 0, 1)
        ->setLore(["§f10 Iron"])
        ->setCustomName("§aStone Sword")
        );
	    $inv->setItem(20, Item::get(Item::IRON_SWORD, 0, 1)
	    ->setLore(["§67 Gold"])
	    ->setCustomName("§aIron Sword")
	    );
	    $inv->setItem(21, Item::get(Item::DIAMOND_SWORD, 0, 1)
	    ->setLore(["§23 Emerald"])
        ...
        // ARMOR //
        if($type == "§fArmor"){
        $inv->setItem(19, Item::get(Item::CHAINMAIL_BOOTS, 0, 1)
        ->setLore(["§f40 Iron"])
        ->setCustomName("§aChainmail Set")
        );
	    $inv->setItem(20, Item::get(Item::IRON_BOOTS, 0, 1)
	    ->setLore(["§612 Gold"])
	    ->setCustomName("§aIron Set")
	    );
	    $inv->setItem(21, Item::get(Item::DIAMOND_BOOTS, 0, 1)
	    ->setLore(["§26 Emerald"])
        ...
        if($type == "§fTools"){
        $inv->setItem(19, Item::get(Item::SHEARS, 0, 1)
        ->setLore(["§f20 Iron"])
	    ...
	    $inv->setItem(20, Item::get(Item::BOW, 0, 1)
	    ->setLore(["§612 Gold"])
	    ->setCustomName("§aBow")
	    );
        ...
        if($type == "§fPotions"){
        $inv->setItem(19, Item::get(373, 16, 1)
        ->setLore(["§21 Emerald"])
        ->setCustomName("§aSpeed Potion II (45 seconds)")
        );
	    $inv->setItem(20, Item::get(373, 11, 1)
	    ->setLore(["§21 Emerald"])
	    ->setCustomName("§aJump Potion IV (45 seconds)")
	    );
	    $inv->setItem(21, Item::get(373, 7, 1)
	    ->setLore(["§22 Emerald"])
        ...
        if($type == "§fUtility"){
        $inv->setItem(19, Item::get(Item::GOLDEN_APPLE, 0, 1)
        ->setLore(["§63 Gold"])
        ->setCustomName("§aGolden Apple")
        );
        $inv->setItem(20, Item::get(Item::SNOWBALL, 0, 1)
        ->setLore(["§f40 Iron"])
        ->setCustomName("§aBedbug")
        );
        $inv->setItem(21, Item::get(Item::SPAWN_EGG, 14, 1)
        ->setLore(["§f120 Iron"])
        ->setCustomName("§aDream Defender")
        );
        $inv->setItem(22, Item::get(Item::FIREBALL, 0, 1)
        ->setLore(["§f40 Iron"])
        ->setCustomName("§aFireball")
        ); 
        $inv->setItem(23, Item::get(Block::TNT, 0, 1)
        ->setLore(["§68 Gold"])
        ->setCustomName("§aTNT")
        );
        $inv->setItem(24, Item::get(Item::ENDER_PEARL, 0, 1)
        ->setLore(["§24 Emerald"])
        ->setCustomName("§aEnder Pearl")
        );
        $inv->setItem(25, Item::get(Item::COMPASS, 0, 1)
        ->setLore(["§23 Emerald"])
        ->setCustomName("§aPlayer Tracker")
        ); 
        $inv->setItem(28, Item::get(Item::BUCKET, 1, 1)
        ->setLore(["§64 Gold"])
    ...
    public function buyItem(Item $item, Player $player){
     ...
     $level->broadcastLevelSoundEvent(new Vector3($player->getX(), $player->getY(), $player->getZ()), LevelSoundEventPacket::SOUND_NOTE); 
        ...
        if ($player->getInventory()->contains($payment)) {
            ...
            if(in_array($item->getCustomName(), ["§aMagic Milk", "§aBedbug", "§aDream Defender", "§aFireball", "§aInvisibility Potion (30 seconds)", "§aSpeed Potion II (45 seconds)", "§aJump Potion IV (45 seconds)"])){
